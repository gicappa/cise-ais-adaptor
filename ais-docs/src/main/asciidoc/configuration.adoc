...
a CR/LF but the it's possible to configure it otherwise. In some cases there is no specific character to delimit messages so it should be used the `!` char.
ais-source.delimiter.char=\n
